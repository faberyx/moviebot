...
const cleanValue = (fieldvalue) => {
  const value = `${fieldvalue}`
    .replace(/\ba\b|\bthe\b|\babout\b|\bby\b|\bwith\b|\bof\b|\bfrom\b/gm, "")
    .trim();
  const ftValue = `+"${value}"`
    .replace(/\band\b\s+/gm, '"+"')
    .replace(/\bor\b\s+/gm, '" "')
...
const getField = (field, fieldvalue, condition = "AND") => {
  if (fieldvalue) {
    ...
    switch (field) {
      case "release":
        query = ` ${condition}   \`release\` BETWEEN ${mysql.escape(
          fieldvalue.from
        )} AND ${mysql.escape(fieldvalue.to)}`;
        break;
      case "decade":
        query = ` ${condition}  \`release\` BETWEEN ${mysql.escape(
          v.value + "-01-01"
        )} AND ${mysql.escape(parseInt(v.value, 10) + 10 + "-01-01")}`;
        break;
      case "year":
        query = ` ${condition}  \`release\` BETWEEN ${mysql.escape(
          v.value + "-01-01"
        )} AND ${mysql.escape(parseInt(v.value, 10) + 1 + "-01-01")}`;
        break;
      case "cast":
        query = ` ${condition}   MATCH (cast) AGAINST (${mysql.escape(
          v.ftValue
        )} IN BOOLEAN MODE)`;
        break;
      case "keywords":
        query = ` ${condition}   MATCH (keywords, title ) AGAINST (${mysql.escape(
          v.ftValue
        )} IN BOOLEAN MODE)`;
        break;
      case "genre":
        query = ` ${condition}   MATCH (genresearch) AGAINST (${mysql.escape(
          v.ftValue
        )} IN BOOLEAN MODE)`;
        break;
      case "certification":
        query = ` ${condition}  lower(\`${field}\`) = ${mysql.escape(
          v.value.toLowerCase()
        )}`;
        break;
      default:
        query = ` ${condition}  lower(\`${field}\`) LIKE ${mysql.escape(
          "%" + v.value.toLowerCase() + "%"
        )}`;
        break;
    }
...
const getCondition = (
  ...
  return `${getField("genre", genre, exclude ? "NOT" : "AND")} ${getField(
    "decade",
    parseInt(decade, 10)
  )} ${getField("keywords", keyword)} ${getField(
    "director",
    director
  )} ${getField("cast", cast)} ${getField("country", country)} ${getField(
    "country",
    country
  )} ${getField("release", releaseTime)} ${getField("year", year)}  ${getField(
    "certification",
    certification
  )}`
    .trim()
...
module.exports.getMovieList = async (
  ...
  try {
    ...
    const query = `
      ...
      SELECT id, title, genre, director, country, \`release\`, img, vote ${rank} FROM moviesdb.movies WHERE ${condition} ORDER BY ${
...
module.exports.getSearchGlobal = async (searchGlobal, offset, limit) => {
  try {
    const query = `SELECT COUNT(*) as total FROM moviesdb.movies WHERE ${getGlobalCondition(
      searchGlobal
    )};SELECT  id, title, genre, director, country, \`release\`, img,  vote  FROM moviesdb.movies WHERE ${getGlobalCondition(
      searchGlobal
...
const getRank = (cast, genre, keyword) => {
  if (cast) {
    ...
    return `, MATCH (cast) AGAINST (${mysql.escape(
      v.ftValue
    )} IN BOOLEAN MODE) as rank`;
  }
  if (keyword) {
    const v = cleanValue(keyword);
    return `, MATCH (keywords, title ) AGAINST (${mysql.escape(
      v.ftValue
    )} IN BOOLEAN MODE) as rank`;
  }
