 ...
 */
const getCondition = (genre, decade, keyword, director, cast, country, releaseTime, year, certification, exclude = null) => {
  return `${getField('genre', genre, exclude ? 'NOT' : 'AND')} ${getField('decade', parseInt(decade, 10))} ${getField('keywords', keyword)} ${getField('director', director)} ${getField('cast', cast)} ${getField(
    'country',
    country
  )} ${getField('country', country)} ${getField('release', releaseTime)} ${getField('year', year)}  ${getField('certification', certification)}`
    .trim()
    .substring(4);
};
 ...
 * @param {number} limit
 */
module.exports.getMovieList = async (genre, decade, keyword, director, cast, country, releaseTime, year, certification, exclude, offset, limit) => {
  try {
    const rank = getRank(cast, genre, keyword);
    const condition = getCondition(genre, decade, keyword, director, cast, country, releaseTime, year, certification, exclude);
